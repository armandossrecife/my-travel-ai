...
function renderCosts(plan) {
  ...
  const items = [
    ...
    { icon: '💰', label: 'Total Estimado', value: ec.total_estimado, highlight: true },
  ];
...
function renderFlights(passagensResult) {
  ...
  const flightCards = opcoes.map((op, i) => {
    const isBest = op.companhia === bestCompanhia && i === 0;
    const escalasText = op.escalas === 0 ? '🟢 Direto' : op.escalas === 1 ? '🟡 1 escala' : `🔴 ${op.escalas} escalas`;
    return `
...
function renderHotels(hoteisResult) {
  ...
  const hotelCards = opcoes.map((h, i) => {
    ...
    const highlightTags = (h.destaques || []).slice(0, 4).map(d => `<span class="highlight-tag">${d}</span>`).join('');
...
function renderItinerary(turismoResult) {
  ...
  const dayCards = roteiro.map(dia => {
    const dataObj = new Date(dia.data + 'T12:00:00');
    const dateStr = dataObj.toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long' });

    const timeBlocks = ['manha', 'tarde', 'noite'].map(period => {
...
function showSection(name) {
  [agentsSection, resultsSection, errorSection].forEach(s => s?.classList.add('hidden'));
...
function validateForm(data) {
  ...
  if (!data.cidade_destino.trim()) {
    document.getElementById('cidade_destino')?.closest('.form-group')?.classList.add('has-error');
    valid = false;
  }
  if (!data.data_saida) {
    document.getElementById('data_saida')?.closest('.form-group')?.classList.add('has-error');
    valid = false;
  }
  if (!data.data_retorno) {
    document.getElementById('data_retorno')?.closest('.form-group')?.classList.add('has-error');
    valid = false;
  }
  if (data.data_saida && data.data_retorno && data.data_retorno <= data.data_saida) {
    document.getElementById('data_retorno')?.closest('.form-group')?.classList.add('has-error');
    valid = false;
  }
...
form.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    else errs = ['Erro inesperado ao processar a solicitação. Tente novamente.'];

    messages.innerHTML = errs.map(e => `<div class="error-message">${e}</div>`).join('');
